...
function LoginComponent(){
    ...
    const login = async (e) =>{
        e.preventDefault();
        try {
            let model = {email: email, password: password};   //obje oluşturalım.
            let response = await axios.post("http://localhost:5000/auth/login", model);  //logine api isteği atıcam.response dönecek.
            localStorage.setItem("token", response.data.token);   //token içerisine token gelecek.
