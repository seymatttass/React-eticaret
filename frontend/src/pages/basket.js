...
function BasketComponent(){
    ...
    const [total, setTotal] = useState(null); //Toplam tutar.

    const getAll = async() =>{
        let user = JSON.parse(localStorage.getItem("user")); //user elde edelim.
        let model = {userId: user._id};  //objeye dönüştürleim userid yi.
        let response = await axios.post("http://localhost:5000/baskets/getAll",model);
        setBaskets(response.data);  
        let totalC = 0;               //Toplam tutar.
